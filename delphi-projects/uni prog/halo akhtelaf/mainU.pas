...
begin
  DataM.Querym.Active := False;
  DataM.Querym.SQL.Clear;
  DataM.Querym.SQL.Add('select * from dadnamedadkhast');
  DataM.Querym.Active := True;
...
begin
 DataM.Querym.Active := False;
  DataM.Querym.SQL.Clear;
  DataM.Querym.SQL.Add('select * from dadkhast where clase in (select R4dadnumber from tajdid ) and  clase not in (select clase from dadnamedadkhast  )');
  DataM.Querym.Active := True;
