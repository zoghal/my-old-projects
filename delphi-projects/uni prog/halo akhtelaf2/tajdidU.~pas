...
begin
  ...
  if  DataM.dadkhastT.FindKey([ Edit1.Text]) then // çò ò—œ‰ „ÊÃÊœ »Êœ‰ œ«œŒÊ«” 
      ...
      datam.Querym.SQL.Add('select * from dadnamedadkhast where Clase='+#39+Edit1.Text+#39+'and type='+#39+'dad'+#39);
      datam.Querym.Active :=true;
      If DataM.Querym.RecNo = 0 then
      begin
         DataM.dadkhastT.Active := false;
         ShowMessage('Å—Ê‰œÂ Â‰Ê“ „Œ Ê„Â ‰‘œÂ');
         Exit;
      End;
...
begin
    ...
    DataM.tajdidT.Post;
      ShowMessage('‘„«—Â ò·«”Â :'+ datam.tajdidTCodet.AsString);
    Datam.tajdidT.Active := false;
    tajdidform.Close;
end;
